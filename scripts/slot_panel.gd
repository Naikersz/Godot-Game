...
func _can_drop_data(_position: Vector2, data: Variant) -> bool:
	...
	if not manager.has_method("slot_can_drop_data"):
		return false
	# Wir verarbeiten Drops explizit per Klick, nicht per Loslassen.
	if DragState.active:
		return false
	...
	if DragState.active and DragState.source_kind == "world" and not DragState.get_item().is_empty():
		# Erstelle temporäres Dictionary für Prüfung
		var temp_data = {
			"item": DragState.get_item(),
			"source_kind": DragState.source_kind,
			"source_id": DragState.source_id
		}
		return manager.slot_can_drop_data(self, temp_data)
...
func _gui_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		...
		# Linksklick für Welt-Loot - nur wenn Drag aktiv ist
		if mb.button_index == MOUSE_BUTTON_LEFT and not mb.pressed:
			# Nur reagieren, wenn ein Drag aktiv ist (Welt-Loot oder Inventar)
		...
		# Mittlere Maustaste für Inventar-Sortierung
